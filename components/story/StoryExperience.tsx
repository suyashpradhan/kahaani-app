...
export function StoryExperience({ shareToken }: { shareToken: string }) {
  ...
  return <main className="story-shell"><header className="story-head"><div className="wordmark">Kahaani</div></header>
    {screen === "invite" && <div className="story-main"><div className="story-host">{text.hostIntro(invitation.hostName)}</div><div className={`prompt-question ${storyClass}`}>{invitation.questionLocalized}</div>{invitation.promptAudioUrl && <><audio ref={audioRef} src={invitation.promptAudioUrl}/><button className="hear-button" onClick={()=>void audioRef.current?.play()}><Icon name="sound" size={28}/>{text.generatedPrompt(invitation.hostName)}</button></>}<div style={{marginTop:"auto"}}><button className="primary story-primary" onClick={startRecording}><Icon name="mic" size={40}/>{text.record}</button></div></div>}
